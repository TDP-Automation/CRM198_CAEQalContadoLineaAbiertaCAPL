...
Sub CambioSimplificado()
 ...
 @@ hightlight id_;_29628800_;_script infofile_;_ZIP::ssf8.xml_;_
	...
	If ucase(DataTable("e_MetodoEntrega", dtlocalSheet)) = "DELIVERY" Then
		Call InstruccionesEnvio() 
	End If
	If (ucase(DataTable("e_FormaPago", dtlocalSheet)) = "SI" or ucase(DataTable("e_Financiamiento", dtlocalSheet)) = "SI") Then
		Call NegociarPago()
	End If
...
Sub NPC()
	...
	Call Carga()
	wait 2
	If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaTable("SearchJTable").Exist = true Then
	...
	imagenToWord "Negociar Configuración de Producto",RutaEvidencias() &"NPC.png"
	wait 1
...
Sub NegociarPago()
		...
		wait 1
		Dim x
		x = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 833910A").JavaEdit("ID del cliente:").GetROProperty("text")
		While x = ""
			wait 1
			x = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 833910A").JavaEdit("ID del cliente:").GetROProperty("text")
		Wend
		...
		If ucase(DataTable("e_MetodoEntrega",dtLocalSheet)) = "DELIVERY" Then
			Dim f, g
			f = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 833910A").JavaEdit("CIP").GetROProperty("enabled")
			g = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 833910A").JavaEdit("CIP").GetROProperty("text")
			If f = "0" and g = ""  Then
				JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &"CIP.png", True
				imagenToWord "VALIDACION COMPLETA: CIP se encuentra deshabilitado",RutaEvidencias() &"CIP.png"
				DataTable("s_Resultado", dtlocalSheet) = "EXITO"
				DataTable("s_Detalle", dtlocalSheet) = "CIP se encuentra deshabilitado"
				Reporter.ReportEvent micPass, DataTable("s_Resultado", dtlocalSheet), DataTable("s_Detalle", dtlocalSheet)
			else
				JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &"CIP.png", True
				imagenToWord "Validación incorrecta",RutaEvidencias() &"CIP.png"
				DataTable("s_Resultado", dtlocalSheet) = "Fallido"
				DataTable("s_Detalle", dtlocalSheet) = "Validación incorrecta"
				Reporter.ReportEvent micPass, DataTable("s_Resultado", dtlocalSheet), DataTable("s_Detalle", dtlocalSheet)
			End If
		End If
